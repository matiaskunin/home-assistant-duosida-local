...
- [`duosida-local`](https://github.com/matiaskunin/duosida-local) for the local
  protocol implementation, evidence levels and hardware sources.

The integration contains no code from DSCharge or from third-party Duosida
integrations. Its runtime calls only the separately packaged clean-room library.
The original charger photographs and private captures are not repository assets.
